...
def build_input_features(dataset):
    for subject in dataset.subjects():
        for action in dataset[subject].keys():
            action = dataset[subject][action]

            positions_world = action['positions_world']
            action['future_trajectory_ground_20'] = positions_world[20:-40, 0, [0, 2]]
            action['future_trajectory_ground_40'] = positions_world[40:-20, 0, [0, 2]]
            action['future_trajectory_ground_60'] = positions_world[60:, 0, [0, 2]]
            ...
            action['future_facing_direction_20'] = facing_direction[20:-40]
            action['future_facing_direction_40'] = facing_direction[40:-20]
            action['future_facing_direction_60'] = facing_direction[60:]
            
            action['local_foot_positions'] = action['positions_local'][:, [lf, rf]]
            action['local_foot_velocities'] = action['velocities_local'][:, [lf, rf]]
            action['local_hip_velocity'] = action['velocities_local'][:, 0]


def compute_velocities(dataset):
    fps = dataset.fps()
    for subject in dataset.subjects():
        for action in dataset[subject].keys():
            action = dataset[subject][action]
